...
export async function calculateNutritionForServing(
  serving: Serving,
): Promise<Nutrition> {
...
export function calculateNutritionForItemAndServing(
  item: Item,
  serving: Serving,
): Nutrition {
  ...
  return {
    ...
    carbs: carbsGrams,
    cals:
      proteinGrams * CALS_PER_GRAM_PROTEIN +
      carbsGrams * CALS_PER_GRAM_CARBS +
      fatGrams * CALS_PER_GRAM_FAT,
  };
}
